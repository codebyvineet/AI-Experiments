...
async def create_item(
    ...
    """Create a new item."""
    return await item_crud.create_item(item_data, current_user.user_id)
...
async def list_items(
    ...
    """List all items, or only the current user's items."""
    owner_id = current_user.user_id if my_items_only else None
    return await item_crud.list_items(owner_id=owner_id, skip=skip, limit=limit)
...
async def update_item(
    ...
    """Update an item (only the owner may update)."""
